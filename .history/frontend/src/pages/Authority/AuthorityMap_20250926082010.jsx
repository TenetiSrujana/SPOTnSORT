// src/pages/Common/ReportsMap.jsx
...
// Custom arrow marker (optional)
const arrowIcon = new L.Icon({
  iconUrl: "/arrow-marker.png", // put arrow marker in public folder
  iconSize: [35, 35],
  iconAnchor: [17, 35],
  popupAnchor: [0, -35],
});
...
export default function ReportsMap({ role }) {
  ...
  useEffect(() => {
    ...
    const handleStorage = () => fetchReports();
    window.addEventListener("storage", handleStorage);

    return () => window.removeEventListener("storage", handleStorage);
  }, []);

  // Status color: Pending = red, Resolved = green
        ...
        >
          ...
          {displayedReports.map(
            (r) =>
              r.lat &&
              r.lng && (
                <Marker key={r.id} position={[r.lat, r.lng]} icon={arrowIcon}>

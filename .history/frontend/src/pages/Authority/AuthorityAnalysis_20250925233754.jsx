...
const Analytics = () => {
  ...
  const statusData = [
    { name: "Pending", value: reports.filter((r) => r.status !== "Completed").length },
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <main className="pt-32 px-6 pb-24 max-w-6xl mx-auto space-y-16">
        ...
        <section className="bg-black bg-opacity-70 p-6 rounded-xl shadow-lg">
          ...
          {reports.slice(-5).reverse().map((r) => (
            <div
              key={r.id}
              className="border-2 border-gray-600 rounded p-2 mb-2 flex justify-between items-center"
            >
              <div>
                <p>
                ...
                <p>
                  <small>Reported by: {r.name} ({r.userEmail})</small>
                </p>
              </div>
      ...
      {/* Arrow button to go to home page */}
      <button
        onClick={() => navigate("/authority/home")}
        className="fixed top-6 right-6 p-3 bg-yellow-500 text-black rounded-full shadow-lg hover:bg-yellow-600 transition"
        title="Go to Home"
      >
        <FaArrowUp />
      </button>

...
import { FaClipboardList, FaExclamationTriangle, FaMapMarkerAlt, FaLightbulb, FaCity, FaCheckCircle } from "react-icons/fa";
  ...
  return (
    <div className="relative text-white overflow-y-auto scroll-smooth min-h-screen">
      ...
      <header className="fixed top-0 left-0 w-full z-50">
        <nav className="flex justify-between items-center px-10 py-4 bg-black bg-opacity-40 backdrop-blur-sm shadow-md">
          ...
          <ul className="flex space-x-6 font-semibold text-white">
            {["Home", "Assigned Reports", "Analytics", "Map", "Profile"].map((item, idx) => (
              <li
                key={idx}
                ...
                onClick={() => {
                  switch (item) {
                    ...
                    case "Assigned Reports":
                      navigate("/authority/manage"); // ✅ Fixed path
                      break;
                    case "Analytics":
                      navigate("/authority/analysis"); // ✅ Fixed path
                      break;
                    case "Map":
                      navigate("/authority/map");
                      break;
                    case "Profile":
                      navigate("/authority/home"); // or your profile route
                      break;
                    default:
                      break;
                  }
      ...
      >
        ...
        <button
          onClick={() => navigate("/authority/manage")} // ✅ Fixed path

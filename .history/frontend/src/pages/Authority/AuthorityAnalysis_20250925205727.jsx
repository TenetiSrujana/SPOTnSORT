...
export default function AuthorityAnalysis() {
  ...
  useEffect(() => {
    ...
    const filt = all.filter((r) => {
      const areaMatch =
        !myArea || (r.area && r.area.toLowerCase().includes(myArea.toLowerCase()));
      const catMatch =
        !myCategory || (r.type && r.type.toLowerCase() === myCategory.toLowerCase());
    ...
    const byCat = {};
    filt.forEach((r) => {
      byCat[r.type] = (byCat[r.type] || 0) + 1;
    });

    const byArea = {};
    filt.forEach((r) => {
      byArea[r.area || "Unknown"] = (byArea[r.area || "Unknown"] || 0) + 1;
    });
    ...
    if (lineRef.current) {
      new Chart(lineRef.current, {
        ...
        data: {
          labels: dates,
          datasets: [
            { label: "Reports", data: counts, fill: false, borderColor: "#FFD700", tension: 0.2 },
          ],
        },
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-50">
      <div className="max-w-6xl mx-auto bg-white p-4 rounded shadow">
        ...
        {/* Button to go to Assigned Reports */}
        <button

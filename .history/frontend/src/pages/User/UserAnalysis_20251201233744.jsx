...
export default function UserAnalysis() {
  ...
  const handleRating = async (id, rating) => {
    try {
      await fetch(`http://localhost:5001/api/reports/${id}/rating`, {
        method: "PUT",
  ...
  // COLOR SYNC: Red, Yellow, Green
  const COLORS = ["#e06b6bff", "#FACC15", "#22C55E"];
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <main className="pt-24 p-6">
        ...
        {/* SUMMARY CARDS — Colors Synced */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">

          <div className="bg-red-500 bg-opacity-70 p-4 rounded text-center">
            <h2>Pending</h2>
            <p className="text-2xl font-bold">{pending}</p>
          </div>

          <div className="bg-yellow-400 bg-opacity-70 p-4 rounded text-black text-center">
            <h2>In Progress</h2>
            <p className="text-2xl font-bold">{summary.progress}</p>
          </div>

          <div className="bg-green-500 bg-opacity-70 p-4 rounded text-center">
            <h2>Resolved</h2>
